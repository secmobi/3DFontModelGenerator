...
def openscad_base():
	return """module BezConic(p0,p1,p2,steps=5,h=10) {
...
def ddump_char(font, module, unichar, size=12, height=10):
    charcode = ord(unichar)

    output = openscad_base()

    try:
        points,tags,contourlist,bbox = loadttf(font,charcode,size)
        contours =  split_contours(hex(charcode),module, points,tags,contourlist)
        ...
        output += make_openscad_commands(hex(charcode),module, contours,chunks,bbox,height)
        output += os.linesep + 'translate([0,0,' + str(height) + '/2]) ' + module + '_letter' + hex(charcode) + '();'+os.linesep

        print 'processed:' + unichr(charcode) + ' (' + hex(charcode) + ')'
    except IndexError:
        print '! unable to output character:',hex(charcode),'render:',unichr(charcode)

    return output

def ddump_string(font, module, unistring, size=12, height=10, spacing=10):
    offset = 0
    ...
    for char in unistring:
        ...
        try:
            ...
            chunks = make_chunks(contours)
            output += make_openscad_commands(hex(charcode),module, contours,chunks,bbox,height)
            last_output += os.linesep + 'translate([' + str(offset) + ',0,' + str(height) + '/2]) ' + module + '_letter' + hex(charcode) + '();'+os.linesep

            print 'processed:' + unichr(charcode) + ' (' + hex(charcode) + ')'
        except IndexError:
            print '! unable to output character:',hex(charcode),'render:',unichr(charcode)

        offset = offset + bbox[1][0] + spacing
